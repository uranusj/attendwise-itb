...
function SubjectCard({ subject }: { subject: SubjectAttendance }) {
  ...
  const safeColour = recommendation.kind === "SAFE" ? "#18754E" : recommendation.kind === "RISK" ? "#A65D00" : "#A5293A";
  return (
    <View style={styles.card}>
      <View style={styles.cardTop}><View style={styles.subjectTitleBlock}><Text style={styles.subject}>{subject.subject}</Text><Text style={styles.code}>{subject.subjectCode}</Text></View><View style={styles.percentBlock}><Text style={[styles.percentage, { color: safeColour }]}>{percentage.toFixed(1)}%</Text><Text style={styles.target}>Target 75%</Text></View></View>
      <View style={styles.progressTrack}><View style={[styles.progressFill, { width: `${Math.min(100, percentage)}%`, backgroundColor: safeColour }]} /></View>
